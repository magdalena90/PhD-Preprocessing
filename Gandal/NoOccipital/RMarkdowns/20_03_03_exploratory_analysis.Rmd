---
title: 'Exploratory Analysis for All Lobes but Occipital'
output:
  html_document:
    code_folding: 'hide'
---
...
- Mean expression by sample has to peaks and a heavy right tail (although the difference doesn't seem to be that big when considering the scale of the x axis)
```{r, fig.width=10}
...
- The two peaks in the mean expression density can be explained in part by to ASD and CTL groups and the heavy right tale corresponds to a group of Autism samples. In general, the autism group has a bigger mean and is more spread out than the control group
...
- Samples with autism tend to have a wider dispersion of mean expression with higher values than the control group and a higher level of expression (as we had already seen above)
...
#### PCA
The first principal component seems to separate relatively well the two diagnosis, although not as well as when we were considering all of the samples together
...
Looks exactly the same as the PCA visualisation
...
Low perplexities seem to capture the difference between diagnosis better, although at the end they seem to be capturing another pattern as well, since the samples seem to be grouped in pairs
```{r tsne_samples, warning=FALSE, message=FALSE, fig.width=10, fig.height=10}
perplexities = c(2,5,10,15)
ps = list()
...
The second pattern t-SNE seems to be capturing is the subject the samples belong to! Although not as clearly as when considering all of the samples
```{r, warning=FALSE}
ggplotly(plot_data %>% ggplot(aes(C1, C2, color=Subject_ID)) + geom_point(aes(id=Subject_ID)) + theme_minimal() + 
         theme(legend.position='none') + ggtitle('t-SNE Perplexity=10 coloured by Subject ID'))
...
- 6146 genes (~38%) are significant using a threshold of 0.05 for the adjusted p-value and a without a log Fold Change threshold (keeping the null hypothesis $H_0: lfc=0$)
...
- The Log Fold Changes seem to separate the samples by Diagnosis best are between 0.32 and 0.33 (with these thresholds, the dataset only consists of 16 genes)

- The control sample with ID 31_072014 seems to have a behaviour very similar to the ASD group

```{r, warning=FALSE}
ggplotly(pcas_samps %>% mutate(abs_lfc=ifelse(fc==-1,-1,round(log2(fc),2))) %>% 
         ggplot(aes(PC1, PC2, color=Diagnosis)) + geom_point(aes(frame=abs_lfc, ids=ID), alpha=0.8) + 
...
```{r, warning=FALSE}
ggplotly(pcas_samps %>% mutate(abs_lfc=ifelse(fc==-1,-1,round(log2(fc),2))) %>% 
         ggplot(aes(PC1, PC2, color=Brain_lobe)) + geom_point(aes(frame=abs_lfc, ids=ID), alpha=0.8) + 
         theme_minimal() + ggtitle('Samples PCA plot modifying filtering threshold'))
```
<br>
