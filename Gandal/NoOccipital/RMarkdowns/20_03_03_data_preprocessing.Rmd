---
title: 'Data preprocessing for All Lobes but Occipital'
output:
  html_document:
    code_folding: 'hide'
---
...
- 70% seems to be a good threshold for the minimum percentage of non zeros, so 30% will be the maximum percentage of zeros allowed in a row
...
thresholds = round(100*(65-seq(65,0,-3))/65)
...
- Four outlier samples were found, all Males from the ASD group
...
Found 11 surrogate variables, since there is no direct way to select which ones to pick [Bioconductor answer](https://support.bioconductor.org/p/71447/), kept all of them.
