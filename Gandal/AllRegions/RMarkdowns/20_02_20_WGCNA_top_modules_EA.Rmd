...
for(i in 1:floor(length(scores)/2)){
  i = 2*i-1
  plot_list = list()
  for(j in 1:2){
  ...
  print(subplot(plot_list, nrows=1) %>% layout(annotations = list(
...
rm(i, j, this_row, new_row, plot_list)
```
...
top_genes_together = list(top_genes_1, top_genes_2)

# 1
i=1
plot_data = datExpr[rownames(datExpr) %in% top_genes_together[[i]]$ID,] %>% mutate('ID' = rownames(.)) %>% 
              melt(id.vars='ID') %>% mutate(variable = gsub('X','',variable)) %>%
              left_join(top_genes_together[[i]], by='ID') %>%
              left_join(datMeta %>% dplyr::select(Dissected_Sample_ID, Diagnosis),
                        by = c('variable'='Dissected_Sample_ID')) %>% arrange(desc(importance))
  
ggplotly(plot_data %>% mutate(external_gene_id=factor(external_gene_id, 
                            ...
                            ggplot(aes(external_gene_id, value, fill=Diagnosis)) + geom_boxplot() + theme_minimal() +
                                   xlab(paste0('Top genes for module ', top_modules[i])) + ylab('Level of Expression') +
...
rm(top_genes_together, plot_data, i)
```
