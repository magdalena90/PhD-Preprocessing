...
if(file.exists(file_name)){
  ...
  EA_dataset = data.frame('ensembl_gene_id' = genes_info$ID, module = genes_info$Module)  %>% filter(genes_info$Module!='gray')
  
  # Assign Entrez Gene Id to each gene
  getinfo = c('ensembl_gene_id','entrezgene')
  mart = useMart(biomart='ENSEMBL_MART_ENSEMBL', dataset='hsapiens_gene_ensembl', host='feb2014.archive.ensembl.org')
  biomart_output = getBM(attributes=getinfo, filters=c('ensembl_gene_id'), values=genes_info$ID[genes_info$Module!='gray'], mart=mart)
  
  EA_dataset = biomart_output %>% dplyr::rename('ID' = ensembl_gene_id) %>% left_join(dataset %>% dplyr::select(ID, contains('MM.')), by='ID')
  ...
  for(module in modules){
    ...
    enrichment_GO[[module]] = gseGO(geneList, OrgDb = org.Hs.eg.db, pAdjustMethod = 'bonferroni', verbose = FALSE, seed = TRUE) %>% data.frame
    enrichment_DO[[module]] = gseDO(geneList, pAdjustMethod = 'bonferroni', verbose = FALSE, seed = TRUE) %>% data.frame
    enrichment_DGN[[module]] = gseDGN(geneList, pAdjustMethod = 'bonferroni', verbose = FALSE, seed = TRUE) %>% data.frame
    enrichment_KEGG[[module]] = gseKEGG(geneList, organism = 'human', pAdjustMethod = 'bonferroni', verbose = FALSE, seed = TRUE) %>% data.frame
    enrichment_Reactome[[module]] = gsePathway(geneList, organism = 'human', pAdjustMethod = 'bonferroni', verbose = FALSE, seed = TRUE) %>% data.frame
  }

  # Save enrichment results
  save(enrichment_GO, enrichment_DO, enrichment_DGN, enrichment_KEGG, enrichment_Reactome, file=file_name)
  
  rm(getinfo, mart, biomart_output, gene, module)
}

```
<br>

### GSEA for SFARI Genes
...
getinfo = c('ensembl_gene_id','entrezgene')
mart = useMart(biomart='ENSEMBL_MART_ENSEMBL', dataset='hsapiens_gene_ensembl', host='feb2014.archive.ensembl.org')
biomart_output = getBM(attributes=getinfo, filters=c('ensembl_gene_id'), values=genes_info$ID, mart=mart)

EA_dataset = biomart_output %>% dplyr::rename('ID' = ensembl_gene_id) %>% left_join(dataset %>% dplyr::select(ID, Module, contains('MM.')), by='ID') %>%
             filter(Module != 'gray') %>% dplyr::select(-Module)
...
term2gene = biomart_output %>% left_join(genes_info %>% dplyr::select(ID, `gene-score`), by = c('ensembl_gene_id'='ID')) %>% filter(`gene-score`!='Others') %>%
            dplyr::select(-ensembl_gene_id) %>% mutate('SFARI' = 'SFARI', `gene-score` = paste0('SFARI Score ',`gene-score`)) %>%
            melt(id.vars = 'entrezgene') %>% dplyr::select(value, entrezgene) %>% dplyr::rename('term' = value, 'gene' = entrezgene) %>% distinct
...
for(module in modules){
    ...
    enrichment_SFARI[[module]] = clusterProfiler::GSEA(geneList, pAdjustMethod = 'bonferroni', TERM2GENE = term2gene, pvalueCutoff = 1, maxGSSize = 1000,
                                                      verbose = FALSE, seed = TRUE) %>% data.frame
}

rm(getinfo, values, mart, biomart_output, term2gene, modules, module, geneList, EA_dataset)
```
<br>
