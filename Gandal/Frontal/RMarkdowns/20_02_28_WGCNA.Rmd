---
title: 'Perform WGCNA for Frontal Lobe'
output:
  html_document:
    code_folding: 'hide'
---
...
Load preprocessed dataset (preprocessing code in 20_02_06_data_preprocessing.Rmd)
...
best_power = datExpr %>% t %>% pickSoftThreshold(powerVector = c(1,2,3,4,seq(5,5.5,0.1)), RsquaredCut=0.8)
...
- Leaves less genes without cluster than the algorithm using all the brain regions (433 vs 4288)

- Creates many more clusters than the when using all the brain regions together (388 vs 192)
...
- More genes are left without a cluster than when considering the samples from all the brain regions (229 vs 54)

- Many more modules than when considering all the brain regions (408 vs 59)
